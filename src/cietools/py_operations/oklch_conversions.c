...
static int OKlch_check_and_extract(PyObject *lch, OKlch *lch_color) {
    ...
    if (PySequence_Size(lch) != 3) {
        PyErr_SetString(PyExc_ValueError, "OKlch argument must have exactly 3 elements in sequence");
        return 0; // Failure
    }
...
PyObject *PyOKlch2RGB(PyObject *self, PyObject *arg) {
    OKlch OKlch_color;

    if (!OKlch_check_and_extract(arg, &OKlch_color)) {
        return NULL; // Error is already set inside the check function
    }

    const RGB rgb_color = oklch2rgb(&OKlch_color);
...
PyObject *PyOKlch2OKlab(PyObject *self, PyObject *arg) {
    OKlch OKlch_color;
    ...
    const OKlab oklab_color = oklch2oklab(&OKlch_color);
